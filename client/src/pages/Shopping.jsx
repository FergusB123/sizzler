  ...
  return (
    <div className="screen">
      ...
      <Segmented
        value={mode}
        onChange={setMode}
        options={[{ value: 'home', label: 'At home' }, { value: 'instore', label: 'In store' }]}
      />
      <p className="shop-hint">
        {mode === 'home' ? 'Tick anything you already have — it’ll drop off your in-store list.' : 'Tick items into your trolley as you shop.'}
      </p>

      <div className="shop-progress"><span style={{ width: `${visible.length ? (doneCount / visible.length) * 100 : 0}%` }} /></div>
      ...
      {grouped.map((g) => (
        <div key={g.key} className="shop-group">
          ...
          {g.list.map((item) => {
            ...
            return (
              <button key={item.id} className={`shop-item ${mode} ${checked ? 'checked' : ''}`} onClick={() => toggle(item)}>
                <span className={`shop-check ${checked ? 'on' : ''}`}>{checked ? '✓' : ''}</span>
